...
import pandas as pd
from mixs import mixs, generate_mixs6_object, get_ena_dict
...
def process_matching_ena_checklists(ena_cl_obj, mixs_v6_obj, cl_hl, ena_results_list):
    ...
    mixs_cat_packages_set = remove_exceptions(cl_hl,
                                              set(find_starting_substrings(cl_hl, mixs_v6_obj.get_gsc_packages())))
    ...
    if len(ena_results_list) == 0:
        ...
        for ena_checklist in ena_results_list:
            ...
            # test for synonyms
            ena_syn_matches_dict, ena_syn_matches_set, mixs_syn_matches_set = get_syn_matching_ena_terms_dict(
                checklist_synonym_dict, ena_difference_terms, mixs_difference_terms)
...
def main():
    ...
    log_file_handle.write(f"current ENA checklists:\n {', '.join(ena_cl_obj.get_all_package_list())}\n\n")
    ...
    for cl_hl in ch_hl_list:
        ...
        if len(results_list) == 0:
            ic(f"WARNING: No ENA checklists corresponding to {cl_hl}")
            log_file_handle.write(f"WARNING: No ENA checklists corresponding to  {cl_hl}\n")
            ...
            log_file_handle.write(f"\tENA package={results_list[0]}\n")
            ena_sp_package_term_list = ena_cl_obj.get_gsc_package_name_specific_fields_list(results_list[0])
            log_file_handle.write(
                f"\tENA terms list (total={len(ena_sp_package_term_list)}): {', '.join(ena_sp_package_term_list)}\n")
            mixs_list_missing, ena_list_missing = process_matching_ena_checklists(ena_cl_obj, mixs_v6_obj, cl_hl,
                                                                                  results_list)
